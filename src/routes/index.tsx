              ...
              <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                <div>
                  ...
                  <div className="flex items-center gap-2 mt-1 flex-wrap">
                    <p className="text-xs text-gray-400">Copy and paste into your Lovable editor.</p>
                    {(() => {
                      const ap = phases.find((p) => p.id === activePhaseId);
                      if (!ap?.generatedPrompt) return null;
                      if (ap.source === "fallback") {
                        return <span className="text-[10px] font-black uppercase tracking-wider px-2 py-0.5 rounded bg-amber-500/10 text-amber-300 border border-amber-500/20" title="AI call did not return a prompt — this is a deterministic template built from your DNA. Check credits or retry.">Template fallback</span>;
                      }
                      if (ap.source === "ai") {
                        return <span className="text-[10px] font-black uppercase tracking-wider px-2 py-0.5 rounded bg-emerald-500/10 text-emerald-300 border border-emerald-500/20" title={ap.model ? `Generated by ${ap.model}` : "Generated by AI"}>AI generated{ap.model ? ` · ${ap.model}` : ""}</span>;
                      }
                      return null;
                    })()}
                  </div>
